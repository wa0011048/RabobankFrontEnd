...
/*
 * Config Load custom tasks configuration
 */
...
// --no-livereload parameter (used in serve task in dev env) to disable live
// reload
var isLivereload = true;
if (typeof argv.livereload !== "undefined") {
	isLivereload = false;
}
// --tdd parameter (used with --watch parameter) to enable running unit test on
// each changes
var tdd = false;
...
/*
 * Clean files Remove the dist/ folder
 */
...
/*
 * Copy all file Use in dev environment to build the project It moves files from
 * src/ to dist/ with almost no files transformation
 */
gulp.task('copy-all', [ 'clean', 'jshint' ], function() {
	/*
	 * Starts by processing main index.html file It removes livereload js if
	 * functionality has been disabled and sass stylesheets if using css without
	 * processor
	 */

	// prepare list of files to pipe after processing index.html file
	var otherSrc = [ config.bases.src + '*', config.bases.src + '**/*',
	...
	.pipe(gulpif(!isLivereload || !isWatch, htmlreplace({
		livereload : ""
	}, {
		keepUnassigned : true,
		keepBlockTags : true
...
/*
 * Copy single files to dist folder For example favicon.ico
 */
gulp.task('single-files', [ 'clean' ], function() {
	gulp.src(config.path.singlefiles, {
		base : config.bases.src
	}).pipe(gulp.dest(config.bases.dist));
});

/*
 * jshint task Apply jshint to all src js files
 */
gulp.task('jshint', function() {
	gulp.src(config.path.scripts).pipe(jshint()).pipe(
			jshint.reporter('default'));
});

/*
 * Process scripts, ng annotate, uglify and concatenate them into one output
 * file Use in production environment
 */
...
/*
 * Process html, replace stylesheets and javascript blocks with minified version
 * and minify html Use in production environment
 */
...
/*
 * Process css, minify it and concat them in a single file Use in production
 * environment
 */
...
/*
 * Process fonts Copy fonts from project and libraries in dist/fonts/ folder
 */
gulp.task('fonts', [ 'clean' ], function() {
	gulp.src(config.path.fonts).pipe(gulp.dest(config.bases.dist + 'fonts/'));
});

/*
 * Process images Minify them and copy to dist/img/ folder Use in production
 * environment
 */
...
/*
 * Download the selenium webdriver Use by npm install to download automatically
 * the selenium web driver
 */
gulp.task('webdriver_update', webdriver_update);

/*
 * Run e2e tests The site should be available on localhost:8888 before running
 * this command. Moreover check the configuration of protractor.conf.js file
 * (mainly the selenium driver)
 */
...
/*
 * Build task Different for dev or prod env
 */
